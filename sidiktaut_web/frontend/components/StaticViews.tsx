...
// --- SHARED COMPONENTS (High Performance CSS) ---
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 pt-2 md:pt-4">
        <div className="lg:col-span-2 space-y-4 md:space-y-6">
          <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white flex items-center gap-3">
            <div className="p-2 bg-blue-100 dark:bg-blue-900/30 text-blue-600 rounded-full"><FileText size={18} md:size={20} /></div> Cara Menginstall
          </h3>
          ...
          <div className="w-full aspect-video bg-black/5 rounded-[15px] overflow-hidden shadow-lg border border-gray-100 dark:border-gray-800 relative group">
            <iframe
              ...
              src="https://www.youtube.com/embed/EKdWnPgZkxY"
        ...
        <div className="space-y-6">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
            <div className="p-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 rounded-full"><Code size={20} /></div>
            Persyaratan Sistem
          </h3>
...
export const CliView = memo(function CliView() {
  ...
  const LOGO_CONFIG = {
    tux: {
      mobile: "absolute bottom-[290px] right-[10px] w-[130px] h-[110px] rotate-[5deg]", 
      desktop: "md:absolute md:top-[130px] md:-translate-y-[40px] md:right-[330px] md:w-56 md:h-56 md:rotate-[6deg]"
    },
    python: {
      mobile: "absolute bottom-[-21px] right-[-30px] w-[270px] h-[270px] rotate-[5deg]",
      desktop: "md:absolute md:top-1/2 md:-translate-y-1/2 md:-right-10 md:w-[400px] md:h-[400px] md:rotate-[5deg]"
    }
  };
  ...
  const copyInstall = () => {
      navigator.clipboard.writeText('python sidiktaut.py');
  ...
  return (
    ...
    >
      ...
      <div className="bg-[#1a1a1a] dark:bg-black rounded-[2rem] md:rounded-[2.5rem] p-6 md:p-12 text-white shadow-[0_20px_50px_-10px_rgba(0,0,0,0.15)] flex flex-col md:flex-row items-center gap-8 md:gap-10 border border-gray-800 relative overflow-hidden min-h-[400px] md:min-h-[auto]">
         ...
         <div className={`${LOGO_CONFIG.tux.mobile} ${LOGO_CONFIG.tux.desktop} pointer-events-none select-none z-0`}>
            <motion.img 
                ...
                className="w-full h-full object-contain brightness-200 contrast-50"
            />
         </div>
         ...
         <div className={`${LOGO_CONFIG.python.mobile} ${LOGO_CONFIG.python.desktop} pointer-events-none select-none z-0`}>
            <motion.img 
                ...
                animate={{ opacity: 0.15, scale: 1 }} 
                transition={{ duration: 1, ease: "easeOut", delay: 0.2 }}
                className="w-full h-full object-contain grayscale brightness-200 contrast-50"
            />
         </div>
         ...
         <div className="flex-1 relative z-10 w-full">
            ...
            <p className="text-gray-400 text-sm md:text-lg leading-relaxed max-w-xl mb-6 md:mb-8">
               Tools forensik URL via terminal. Dilengkapi fitur <b>Trace Redirect</b> dan <b>Auto Logging</b> untuk analisis mendalam.
            </p>
            ...
            <div className="flex flex-wrap gap-3">
                <PrimaryBtn 
                    ...
                    className="bg-amber-600 text-white hover:bg-amber-700 border-none shadow-[0_4px_14px_0_rgba(245,158,11,0.2)]"
                />
         ...
         <div className="relative z-10 w-full md:w-auto md:min-w-[320px] mt-8 md:mt-0">
            ...
            <div className="bg-black rounded-2xl md:rounded-[1.5rem] border border-gray-800 p-4 md:p-5 flex items-center justify-between group shadow-lg hover:border-amber-600 transition-colors w-full relative z-20">
               ...
               <div onClick={copyInstall} className="p-2 hover:bg-gray-800 rounded-lg md:rounded-full cursor-pointer transition-colors shrink-0">
                 {installCopied ? <Check size={16} className="text-amber-500"/> : <Copy size={16} className="text-gray-600 group-hover:text-white"/>}
               </div>
            </div>
         </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 md:gap-8 pt-2 md:pt-4">
         <div className="lg:col-span-2 space-y-4 md:space-y-6">
            <h3 className="text-xl md:text-2xl font-black text-gray-900 dark:text-white flex items-center gap-3">
               <div className="p-2 bg-amber-100 dark:bg-amber-900/30 text-amber-600 rounded-full"><Play size={18} md:size={20}/></div> Contoh penggunaan
            </h3>
            <div className="w-full aspect-video bg-black/5 rounded-[15px] overflow-hidden shadow-lg border border-gray-100 dark:border-gray-800 relative group">
                <iframe 
                    className="w-full h-full"
                    src="https://www.youtube.com/embed/EKdWnPgZkxY" 
                    title="Tutorial Penggunaan CLI"
                    allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                    allowFullScreen
                    referrerPolicy="strict-origin-when-cross-origin"
                ></iframe>
         ...
         <div className="space-y-4 md:space-y-6">
            <h3 className="text-lg md:text-xl font-bold text-gray-900 dark:text-white flex items-center gap-3">
               <div className="p-2 bg-gray-100 dark:bg-gray-800 text-gray-600 rounded-full"><FileText size={18} md:size={20}/></div> Arguments
            </h3>
      ...
      {selectedMember ? (
        <motion.div key="detail" variants={detailVariants} initial="hidden" animate="visible" exit="exit" transition={transitionSettings}>
          ...
          <div className="bg-white dark:bg-[#121214] rounded-[2.5rem] md:rounded-[3.5rem] p-6 md:p-12 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-gray-100 dark:border-gray-800 flex flex-col md:flex-row gap-6 md:gap-12 items-center md:items-start">
            <div className={`w-32 h-32 md:w-64 md:h-64 shrink-0 rounded-full p-2 border-[3px] border-${selectedMember.color}-100 dark:border-${selectedMember.color}-900`}>
              <img src={selectedMember.image} alt={selectedMember.name} className="w-full h-full object-cover rounded-full bg-gray-100" loading="lazy" />
            </div>
            <div className="flex-1 text-center md:text-left">
              <span className={`inline-block px-4 py-1.5 md:px-5 md:py-2 rounded-full text-[10px] md:text-xs font-bold uppercase tracking-widest mb-3 md:mb-4 bg-${selectedMember.color}-100 text-${selectedMember.color}-700 dark:bg-${selectedMember.color}-900/30 dark:text-${selectedMember.color}-400`}>
                {selectedMember.role}
              </span>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-2 md:gap-3 mb-6 md:mb-8">
                {selectedMember.skills.map((skill: string) => (
                  <span key={skill} className="px-4 py-2 md:px-5 md:py-2.5 bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-gray-700 rounded-full text-[10px] md:text-xs font-bold text-gray-600 dark:text-gray-300 uppercase tracking-wide">{skill}</span>
                ))}
              </div>
        ...
        <motion.div key="grid" variants={gridVariants} initial="hidden" animate="visible" exit="exit" transition={transitionSettings} className="space-y-6 md:space-y-8">
          <div className="text-center mb-6 md:mb-8">
            <h1 className="text-2xl md:text-3xl font-black text-gray-900 dark:text-white mb-2">Meet our <span className="text-blue-600">Team</span></h1>
            <p className="text-sm md:text-base text-gray-500 dark:text-gray-400">Tim dibalik pengembangan SidikTaut.</p>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 max-w-5xl mx-auto">
            {teamMembers.map((member) => (
              <motion.div key={member.name} layoutId={member.name} onClick={() => setSelectedMember(member)} className="group cursor-pointer bg-white dark:bg-[#121214] rounded-[2rem] md:rounded-[3.5rem] p-6 md:p-8 border border-gray-100 dark:border-gray-800 hover:border-blue-500 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all flex flex-col items-center text-center relative overflow-hidden" whileHover={{ y: -5 }}>
                <div className="w-24 h-24 md:w-32 md:h-32 rounded-full p-1.5 mb-4 md:mb-6 relative z-10 bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-gray-700">
...
function StepItem({ number, title, children }: any) {
  return (
    <div className="flex gap-4 md:gap-5">
      <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 flex items-center justify-center font-black text-xs md:text-sm shrink-0 border border-blue-100 dark:border-blue-800">{number}</div>
      <div>
        <h4 className="font-bold text-gray-900 dark:text-white text-sm md:text-base mb-1">{title}</h4>
        <p className="text-xs md:text-sm text-gray-500 dark:text-gray-400 leading-relaxed">{children}</p>
      </div>
    </div>
  )
}
...
function SpecItem({ label, value }: any) {
  return (
    <li className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-1 sm:gap-4 border-b border-gray-200 dark:border-gray-700/50 pb-3 last:border-0 last:pb-0">
      <span className="text-gray-500 dark:text-gray-400 font-medium text-xs uppercase tracking-wide sm:normal-case sm:tracking-normal">{label}</span>
      <span className="font-bold text-gray-900 dark:text-white text-sm sm:text-right">{value}</span>
    </li>
  )
}

function CodeBlock({ title, cmd }: any) {
  ...
  return (
    <div className="group">
      <p className="text-[10px] md:text-xs font-bold text-gray-400 mb-2 ml-2 uppercase tracking-wider">{title}</p>
      <div className="bg-[#0a0a0a] rounded-2xl md:rounded-[1.5rem] border border-gray-800 p-4 md:p-5 font-mono text-xs md:text-sm text-amber-400 flex justify-between items-center hover:border-gray-600 transition-colors shadow-inner">
        <span className="break-all mr-2">{cmd}</span>
        <div onClick={handleCopy} className="p-2 cursor-pointer transition-colors hover:text-white shrink-0">
          {isCopied ? <Check size={16} className="text-amber-500" /> : <Copy size={16} className="text-gray-600 group-hover:text-white" />}
        </div>
      </div>
    </div>
  )
...
function ArgRow({ flag, desc }: any) {
  return (
    <tr className="border-b border-gray-100 dark:border-gray-800/50 last:border-0 hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
      <td className="px-4 py-3 md:px-6 md:py-4 font-mono text-[10px] md:text-xs text-gray-700 dark:text-gray-300 font-bold whitespace-nowrap">{flag}</td>
      <td className="px-4 py-3 md:px-6 md:py-4 text-gray-600 dark:text-gray-400 text-xs md:text-sm">{desc}</td>
    </tr>
  )
}
